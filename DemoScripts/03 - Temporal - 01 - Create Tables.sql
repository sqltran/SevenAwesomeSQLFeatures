-- use master;
-- go
-- alter database CompanyHR_SV set offline with rollback immediate;
-- alter database CompanyHR_SV set online with rollback immediate;
-- drop database if exists CompanyHR_SV;
use master;
go
create database CompanyHR_SV;
go
use CompanyHR_SV;
go
...
-- Anything in history yet?  (Fix the name of the department history table.)
...
-- Check again (and fix the table name for department history)
select * from MSSQL_TemporalHistoryFor_565577053;
select * from EmployeeHistory;
